...
test("introduces instructions and the three ordered stages without prior jargon", () => {
  ...
  expect(screen.getByText("Clock tick", { exact: true })).toBeVisible();
  expect(screen.getByText(/not a whole instruction cycle/i)).toBeVisible();
...
test("clock explanation is distinct, and reset and language changes preserve the right state", async () => {
  const user = userEvent.setup();
  render(<App />);
  expect(screen.getByRole("img", { name: "A visual showing regular clock timing pulses" })).toBeVisible();
  expect(screen.getByText(/clock provides regular timing signals/i)).toBeVisible();
  await user.click(screen.getByRole("button", { name: "Fetch the next instruction" }));
