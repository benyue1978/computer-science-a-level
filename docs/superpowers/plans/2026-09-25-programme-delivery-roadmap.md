...
- [ ] Before the user trial, test the production build locally with `npm run build` and `npm run preview`, including media, reload and direct navigation. The first browser build has a home page at `/` and the memory activity at `/learn/memory`. Configure the static-host fallback so lesson links work when opened directly or refreshed; later lessons and the simulator join this same app.
